-- This script contains the DBFS user-session based trigger implemented within Oracle CMSDK. On each CMSDK user session creation and termination, the session-based trigger is used to call decryption and encryption procedure respectively. 

-- Those two procedures first obtain the single-user as well as the multi-user GIS files to which user has privilege, and thencalls either the decryption or the encryption procedure so that these files can be decrypted or encrypted. 
