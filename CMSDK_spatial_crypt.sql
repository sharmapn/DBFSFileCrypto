...
-- The procedures within this package are executed upon GIS-DFS user session creation and termination in CMSDK. 
-- The CMSDK Spatial Crypt Package obtains details of all files belonging to individual CMSDK spatial users and
...
CREATE OR REPLACE PACKAGE CMSDK_spatial_crypt AS
  -- Retrieve user specific and file specific cryptographic keys
  FUNCTION get_enc_key (user_id INTEGER) RETURN VARCHAR;  -- returns CMSDK user's file's encryption key
  FUNCTION get_dec_key (user_id INTEGER) RETURN VARCHAR;  -- returns CMSDK user's file's decryption key
  FUNCTION get_file_enc_key (filepath VARCHAR2) RETURN VARCHAR; -- returns encryption key FOR that CMSDK file
  FUNCTION get_file_dec_key (filepath VARCHAR2) RETURN VARCHAR; -- returns decryption key FOR that CMSDK file
  -- Split filepath to get file id and test file usage procedures
  FUNCTION split (p_filepath VARCHAR2) RETURN INTEGER;           -- splits CMSDK filepath and return fileid
  FUNCTION test_in_use (v_filepath IN VARCHAR, v_user_id IN INTEGER) RETURN BOOLEAN; -- test file usage
  -- Compute list of files that need to be encrypted and decrypted for an FS user
  PROCEDURE get_10gcmsdk_user_files_enc (v_userid IN INTEGER);       -- obtain CMSDK user's files to be encrypted
  PROCEDURE get_10gcmsdk_user_files_dec (v_userid IN INTEGER);       -- obtain CMSDK user's files to be decrypted
...
-- copy modified blobs which do not need encryption   
  ...
  END CMSDK_spatial_crypt;
/
-- Program definitions in package CMSDK_spatial_crypt
CREATE OR REPLACE PACKAGE BODY CMSDK_spatial_crypt IS

  -- The key_storage table contains CMSDK user and their encryption and decryption keys
  -- This function queries and returns the encryption key based on the CMSDK user_id passed as parameter
  FUNCTION get_enc_key (user_id INTEGER) RETURN VARCHAR IS
  v_enc_key VARCHAR(50);
  BEGIN
    SELECT enc_key INTO v_enc_key FROM CMSDK_UserSpecificFile_keys WHERE CMSDK_user_id = user_id;
    RETURN v_enc_key;
  END;

  -- The key_storage table contains CMSDK user and their encryption and decryption keys
  -- This function queries and returns the decryption key based on the CMSDK user_id passed as parameter
  FUNCTION get_dec_key (user_id INTEGER) RETURN VARCHAR IS
  v_dec_key VARCHAR(50);
  BEGIN
    SELECT dec_key INTO v_dec_key FROM CMSDK_UserSpecificFile_keys WHERE CMSDK_user_id = user_id;
    RETURN v_dec_key;
  END;

  -- The CMSDK_MultiUserFile_keys_state table contains CMSDK file and their encryption and decryption keys
  -- This function computes the encryption key for the CMSDK file with CMSDK filepath passed as parameter
  FUNCTION get_file_enc_key (filepath VARCHAR2) RETURN VARCHAR IS
  v_enc_key VARCHAR(50);
  BEGIN
    SELECT enc_key INTO v_enc_key FROM CMSDK_MultiUserFile_keys_state WHERE CMSDK_filepath = filepath;
    RETURN v_enc_key;
  END;

  -- The CMSDK_MultiUserFile_keys_state table contains CMSDK file and their encryption and decryption keys
  -- This function computes the decryption key for the CMSDK file with CMSDK filepath passed as parameter
  FUNCTION get_file_dec_key (filepath VARCHAR2) RETURN VARCHAR IS
  v_dec_key VARCHAR(50);
  BEGIN
    SELECT dec_key INTO v_dec_key FROM CMSDK_MultiUserFile_keys_state WHERE CMSDK_filepath = filepath;
    RETURN v_dec_key;
  END;

  -- This function is passed with CMSDK file filpath and it checks to see if the users who should have access
  ...
  -- particular file.
  FUNCTION test_in_use (v_filepath IN VARCHAR, v_user_id IN INTEGER) RETURN BOOLEAN IS
  CURSOR in_session_files IS
    SELECT cmsdk_filepath FROM CMSDK_MultiUserFile_user_catalog
      WHERE cmsdk_user_id IN (SELECT userid FROM ifssys.odmz_session WHERE userid != v_user_id);
  ...
  BEGIN
    ...
    LOOP
      ...
      IF (in_session_files_record.CMSDK_filepath = v_filepath) THEN -- if file to be encrypted is decrypted
  ...
  -- This procedure is passed with the CMSDK userid and obtains all files to which the CMSDK user
  -- should have access to and call encryption procedure
  PROCEDURE get_10gcmsdk_user_files_enc (v_userid IN INTEGER) IS
  v_enc_key       VARCHAR(50);    -- CMSDK user-specific encryption key
  v_file_enc_key  VARCHAR(50);    -- CMSDK file-specific encryption key
  v_fileid        INTEGER;        -- CMSDK file_id computed from CMSDK filepath
  ...
  -- Set of user-specific sensitive files which were decrypted and need encryption back.
   CURSOR CMSDK_UserSpecific_Mod_Enc IS
     SELECT CMSDK_file_id AS id FROM CMSDK_User_TempLOBS WHERE CMSDK_user_id = v_userid
        INTERSECT
          SELECT id FROM ifssys.odm_publicobject WHERE owner = v_userid AND (name LIKE '%.enc');
   -- Set of files whicc have been newly created files which need for encryption
   CURSOR CMSDK_UserSpecific_New_Enc IS
      SELECT id FROM ifssys.odm_publicobject WHERE owner = v_userid AND (name LIKE '%.enc')
        MINUS
          SELECT CMSDK_file_id AS id FROM CMSDK_User_TempLOBS WHERE CMSDK_user_id = v_userid;
-- Set of user-specific xisting files which have been decrypted, maybe modified but do not need          -- encryption. They will just be copied into the original blob
   CURSOR CMSDK_UserSpecific_Copy IS
      SELECT CMSDK_file_id AS id FROM CMSDK_User_TempLOBS WHERE CMSDK_user_id = v_userid
        INTERSECT
         SELECT id FROM ifssys.odm_publicobject WHERE owner = v_userid AND (name NOT LIKE '%.enc');

  -- generate list of Multi-User sensitive files to which this user has access
  CURSOR CMSDK_MultiUser_filepaths IS
    SELECT i.CMSDK_filepath FROM CMSDK_MultiUserFile_user_catalog i, CMSDK_MultiUserFile_keys_state  f
      WHERE i.CMSDK_filepath = f.CMSDK_filepath
       AND (f.file_encrypted LIKE 'FALSE') AND (i.CMSDK_user_id = v_userid);

  CMSDK_UserSpecific_Mod_Enc_rec        CMSDK_UserSpecific_Mod_Enc%ROWTYPE;
  CMSDK_UserSpecific_New_Enc_rec        CMSDK_UserSpecific_New_Enc%ROWTYPE;
  CMSDK_UserSpecific_Copy_record        CMSDK_UserSpecific_Copy%ROWTYPE;
  CMSDK_MultiUser_filepaths_record      CMSDK_MultiUser_filepaths%ROWTYPE;
  ...
  BEGIN
    ...
    OPEN CMSDK_UserSpecific_Mod_Enc;
    LOOP
      FETCH CMSDK_UserSpecific_Mod_Enc INTO CMSDK_UserSpecific_Mod_Enc_rec;
      EXIT WHEN CMSDK_UserSpecific_Mod_Enc%NOTFOUND;
      -- call encryption procedure passing file_id and encryption key as parameter
      encrypt_User_Specific_file (CMSDK_UserSpecific_Mod_Enc_rec.id, v_enc_key,v_userid, true);
    END LOOP;
    CLOSE CMSDK_UserSpecific_Mod_Enc;
    -- Encrypt Files that were not initially decrypted but need to be encrypted
    OPEN CMSDK_UserSpecific_New_Enc;
    LOOP
      FETCH CMSDK_UserSpecific_New_Enc INTO CMSDK_UserSpecific_New_Enc_rec;
      EXIT WHEN CMSDK_UserSpecific_New_Enc%NOTFOUND;
      -- call encryption procedure passing file_id and encryption key as parameter
      -- pass to intermediate procedure stating that the file previously has not been decrypted
      encrypt_User_Specific_file (CMSDK_UserSpecific_New_Enc_rec.id, v_enc_key,v_userid, false);
    END LOOP;
    CLOSE CMSDK_UserSpecific_New_Enc;

-- Only Copy content back to original LOB location for files which were decrypted but do not need           -- encryption
    OPEN CMSDK_UserSpecific_Copy;
    LOOP
      FETCH CMSDK_UserSpecific_Copy INTO CMSDK_UserSpecific_Copy_record;
      EXIT WHEN CMSDK_UserSpecific_Copy%NOTFOUND;
     -- call encryption procedure passing file_id and encryption key as parameter-
      blob_copy (CMSDK_UserSpecific_Copy_record.id, v_enc_key,v_userid);
    END LOOP;
    CLOSE CMSDK_UserSpecific_Copy;

    -- Files to which user hass access in multi-user files are encrypted
    OPEN CMSDK_MultiUser_filepaths; -- Obtain all files belonging to multi user and call encryption procedure
    LOOP
      FETCH CMSDK_MultiUser_filepaths INTO CMSDK_MultiUser_filepaths_record;
      EXIT WHEN CMSDK_MultiUser_filepaths%NOTFOUND;
      -- check to see if file is not currently being used
      in_use := test_in_use (CMSDK_MultiUser_filepaths_record.CMSDK_filepath, v_userid);
      IF(in_use = TRUE) THEN
        dbms_output.put_line('File ' || CMSDK_MultiUser_filepaths_record.CMSDK_filepath ||' is in-use');
      ELSIF (in_use = FALSE) THEN   -- if not being used then encrypt back for protection
         encrypt_Multi_user_file_sub (CMSDK_MultiUser_filepaths_record.CMSDK_filepath);
      END IF;
    END LOOP;
    CLOSE CMSDK_MultiUser_filepaths;
  END get_10gcmsdk_user_files_enc;

  -- This procedure is passed with the CMSDK userid and obtain all files to which the
  -- CMSDK user should have access to and call decryption procedure
  PROCEDURE get_10gcmsdk_user_files_dec (v_userid IN INTEGER) IS
  v_dec_key       VARCHAR(50);    -- CMSDK user-specific decryption key
  v_file_dec_key  VARCHAR(50);    -- CMSDK file-specific decryption key
  v_fileid        INTEGER;        -- store CMSDK file_id computed from CMSDK filepath

  CURSOR CMSDK_UserSpecific_Files IS
    SELECT name, id FROM ifssys.odm_publicobject WHERE owner = v_userid AND (name LIKE '%.enc');

  CURSOR CMSDK_MultiUser_filepaths IS
    SELECT i.CMSDK_filepath FROM CMSDK_MultiUserFile_user_catalog i, CMSDK_MultiUserFile_keys_state  f
      WHERE i.CMSDK_filepath = f.CMSDK_filepath
       AND (f.file_encrypted LIKE 'TRUE') AND (i.CMSDK_user_id = v_userid);

  CMSDK_UserSpecific_Files_record CMSDK_UserSpecific_Files%ROWTYPE;
  CMSDK_MultiUser_filepaths_record CMSDK_MultiUser_filepaths%ROWTYPE;
  BEGIN
    v_dec_key := get_dec_key (v_userid);
    OPEN CMSDK_UserSpecific_Files;
    LOOP -- First, decrypt single user-owned files
      FETCH CMSDK_UserSpecific_Files INTO CMSDK_UserSpecific_Files_record;
      EXIT WHEN CMSDK_UserSpecific_Files%NOTFOUND;
      INSERT INTO CMSDK_User_TempLOBS (CMSDK_user_id , CMSDK_file_id, CMSDK_file_name) VALUES (v_userid,CMSDK_UserSpecific_Files_record.id, CMSDK_UserSpecific_Files_record.name);
      decrypt_User_Specific_file (CMSDK_UserSpecific_Files_record.id + 1, v_dec_key,v_userid);
      COMMIT; --commit changes
    END LOOP;
    CLOSE CMSDK_UserSpecific_Files;

    OPEN CMSDK_MultiUser_filepaths; -- Obtain all files belonging to multi user and call encryption procedure
    LOOP -- then decrypt multiple user accessible files
      FETCH CMSDK_MultiUser_filepaths INTO CMSDK_MultiUser_filepaths_record;
      EXIT WHEN CMSDK_MultiUser_filepaths%NOTFOUND;
        decrypt_Multi_user_file (CMSDK_MultiUser_filepaths_record.CMSDK_filepath);
    END LOOP;
    CLOSE CMSDK_MultiUser_filepaths;
  END get_10gcmsdk_user_files_dec;
  ...
  -- starting from the root folder until it reaches the file, against the metadata table in CMSDK.
  ...
  BEGIN
    SELECT id INTO root_id FROM ifssys.odm_publicobject WHERE name = 'Root Folder';
    top_id := root_id;
    LOOP
      ...
      IF delimeter_pos > 0 THEN                     -- if any filepaths
        ...
        SELECT id INTO v_id FROM ifssys.ODM_PUBLICOBJECT  WHERE name = currentdirectory
          INTERSECT
            SELECT rightobject FROM ifssys.ODM_RELATIONSHIP WHERE leftobject = top_id;
...
-- This procedure uses the fileid parametsr to find the exact temporary LOB locator which is stored in         -- place of the original permanent LOB in the IFS LOB tables. This locator is then passed to the encrypt    -- procedure eventually so that the file data can be encrypted using the LOB segment which exists in the    -- temporary tablespace
  PROCEDURE encrypt_User_Specific_file (file_id IN INTEGER, enc_key IN VARCHAR,v_userid IN INTEGER, decrypted IN BOOLEAN) IS
  blob_original                 BLOB;                     -- the original bobl in CMSDK tables
  v_media                       INTEGER := 0;
  BEGIN
  	-- find the location of the temporary LOB. The original LOB locator has been replaced with these                     	-- temporary Lobs
     IF (decrypted = FALSE) THEN   -- file is new then take the file locator from the original CMSDK table
      SELECT media INTO v_media FROM ifssys.odm_contentobject WHERE ifssys.odm_contentobject.id = (file_id+1);
      IF (v_media = 2061) THEN
        SELECT nonindexedblob2 INTO blob_original FROM ifssys.odmm_nonindexedstore WHERE ifssys.odmm_nonindexedstore.id = (file_id +2);
      ELSE
        SELECT globalindexedblob INTO blob_original FROM ifssys.odmm_contentstore WHERE ifssys.odmm_contentstore.id = (file_id+2);
      END IF;
     ELSE -- file has been decrypted then LOB locator needs to be retrieved from table for encryption back
      SELECT templob INTO blob_original FROM CMSDK_User_TempLOBS WHERE CMSDK_user_id = v_userid AND CMSDK_file_id = file_id;
    END IF;
    ...
    IF (decrypted = TRUE) THEN   -- If file was decrypted then, delete its entry from table
      DELETE FROM CMSDK_User_TempLOBS WHERE CMSDK_user_id = v_userid AND CMSDK_file_id = file_id;
      COMMIT;
    END IF;
  END encrypt_User_Specific_file;

-- This procedure usess the fileid parametsr to find the exact temporary LOB locator which is stored in       -- place of the permanent LOB in the CMSDK LOB tables. This locator is then passed to the encrypt procedure        -- eventually so that the file data can be encrypted using the LOB segment which exists in the temporary    -- tablespace
  ...
  BEGIN
    SELECT media INTO v_media FROM ifssys.odm_contentobject WHERE ifssys.odm_contentobject.id = file_id;
    IF (v_media = 2061) THEN
        SELECT nonindexedblob2 INTO blob_original FROM ifssys.odmm_nonindexedstore WHERE ifssys.odmm_nonindexedstore.id = (file_id + 1);
    ELSE
        SELECT globalindexedblob INTO blob_original FROM ifssys.odmm_contentstore WHERE ifssys.odmm_contentstore.id = (file_id + 1);
    END IF;
    decrypt (file_id, dec_key);
    -- store the original blob location in database table for use during encryption
    UPDATE CMSDK_User_TempLOBS SET templob = blob_original WHERE CMSDK_user_id = v_userid AND CMSDK_file_id = (file_id -1);
    COMMIT;
...
-- The encrypt procedure for both the user-specific and multi-user files. The CMSDK fileid  and location of    -- original LOB is passed as parameter
  ...
  BEGIN
   SELECT to_number(to_char(CURRENT_TIMESTAMP, 'SSSSSxFF')) INTO start_time FROM dual; -- set start time
    ...
    SELECT media INTO v_media FROM ifssys.odm_contentobject WHERE ifssys.odm_contentobject.id = v_file_id;
    IF (v_media = 2061) THEN -- If file is stored in the nonindexed table
        SELECT nonindexedblob2 INTO blob_temporary FROM ifssys.odmm_nonindexedstore
          WHERE ifssys.odmm_nonindexedstore.id = (v_file_id + 1);
      ...
      UPDATE ifssys.odmm_nonindexedstore SET nonindexedblob2 = blob_original WHERE ifssys.odmm_nonindexedstore.id = (v_file_id + 1);
    ELSE – Encryption for files which can be indexed; stored in the contentstore table
      SELECT globalindexedblob INTO blob_temporary FROM ifssys.odmm_contentstore
        WHERE ifssys.odmm_contentstore.id = (v_file_id + 1);
      ...
      IF v_threshold_loops < 1 THEN --If filesize smaller than data_buffer_cache size
        SELECT globalindexedblob INTO blob_temporary FROM ifssys.odmm_contentstore
          WHERE ifssys.odmm_contentstore.id = (v_file_id + 1) FOR UPDATE;
    ...
    UPDATE ifssys.odmm_contentstore SET globalindexedblob = blob_original WHERE ifssys.odmm_contentstore.id = (v_file_id + 1);
    END IF;
...
-- This procedure is used to copy blob content for user-speciifc decrypted files which were marked for       -- decryption but were not marked for encryption by the use before logoff. The IFS fileid and location of    -- original LOB is passed as parameters
  ...
  BEGIN
    -- determine in which CMSDK table, file is stored, indexble table or nonindexble table
   SELECT templob INTO blob_original FROM CMSDK_User_TempLOBS WHERE CMSDK_user_id = v_userid AND CMSDK_file_id = file_id FOR UPDATE;
   SELECT media INTO v_media FROM ifssys.odm_contentobject WHERE ifssys.odm_contentobject.id = v_file_id +1;
   SELECT to_number(to_char(CURRENT_TIMESTAMP, 'SSSSSxFF')) INTO start_time FROM dual; -- set start time
   IF (v_media = 2061) THEN -- If file is stored in the nonindexed table
      SELECT nonindexedblob2 INTO blob_temporary FROM ifssys.odmm_nonindexedstore WHERE ifssys.odmm_nonindexedstore.id = (v_file_id + 2);
      DBMS_LOB.OPEN(blob_temporary, DBMS_LOB.LOB_READONLY); -- Compute file length for encryption            -- processing
      v_blob_length := DBMS_LOB.GETLENGTH(blob_temporary);
      DBMS_LOB.CLOSE(blob_temporary);
      ...
      IF v_threshold_loops < 1 THEN --If filesize smaller than data_buffer_cache size
        SELECT nonindexedblob2 INTO blob_temporary FROM ifssys.odmm_nonindexedstore
          WHERE ifssys.odmm_nonindexedstore.id = (v_file_id + 2) FOR UPDATE;
        -- Open, Split and Encrypt
        -- if filesize is less than 'v_IO_readwrite_chunksize', read all data in 1 read
        IF v_total_IO_readwrites < 1 THEN
          dbms_lob.read(blob_temporary, v_IO_readwrite_chunksize, Pos, raw_input);
      ...
      ELSE -- If filesize equal to or larger than data_buffer_cache size
        v_readwrites_per_threshold := (v_data_buffer_cache_threshold / v_IO_readwrite_chunksize);
        FOR j IN low .. v_threshold_loops LOOP
          SELECT nonindexedblob2 INTO blob_original FROM ifssys.odmm_nonindexedstore
            WHERE ifssys.odmm_nonindexedstore.id = (v_file_id + 1) FOR UPDATE;
          loops_left_to_do := v_total_IO_readwrites - loops_done;
          IF(loops_left_to_do >= v_readwrites_per_threshold) THEN
            loops_to_do_now := v_readwrites_per_threshold;
    ...
    ELSE – Encryption for files which can be indexed; stored in the contentstore table
      ...
      v_threshold_loops := v_blob_length / v_data_buffer_cache_threshold;
      IF v_threshold_loops < 1 THEN --If filesize smaller than data_buffer_cache size
        SELECT globalindexedblob INTO blob_original FROM ifssys.odmm_contentstore
          WHERE ifssys.odmm_contentstore.id = (v_file_id + 1) FOR UPDATE;
        -- if filesize is less than 'v_IO_readwrite_chunksize', read all data in one read
        IF v_total_IO_readwrites < 1 THEN
          dbms_lob.read(blob_temporary, v_IO_readwrite_chunksize, Pos, raw_input);
  ...
  DBMS_LOB.FREETEMPORARY (blob_temporary); -- free the temporary lobs created during the session
  DELETE FROM CMSDK_User_TempLOBS WHERE CMSDK_user_id = v_userid AND CMSDK_file_id = v_file_id;
  COMMIT;
...
-- This procedure is used to decrypt blob content for user-speciifc as well as multi-user files which were -- marked for decryption. The CMSDK fileid and location of original LOB is passed as parameters
  ...
  BEGIN
    ...
    IF (v_media = 2061) THEN -- If file is stored in the nonindexed table
      ...
      UPDATE ifssys.odmm_nonindexedstore SET nonindexedblob2 = blob_temporary WHERE ifssys.odmm_nonindexedstore.id = (v_file_id + 1);
    ...
    ELSE -- If file is stored in the contentstore table
      ...
      UPDATE ifssys.odmm_contentstore SET globalindexedblob = blob_temporary WHERE ifssys.odmm_contentstore.id = (v_file_id + 1);
    END IF;
    time_taken := (to_number(to_char(CURRENT_TIMESTAMP, 'SSSSSxFF')) - start_time);
    dbms_output.put_line('TIME TAKEN > ' || time_taken);
  COMMIT;
  ...
  BEGIN
    ...
    ELSE                      -- if file is indexble and therefore stored in odmm_contentstore table
      SELECT globalindexedblob INTO blob_locator FROM ifssys.odmm_contentstore
        WHERE ifssys.odmm_contentstore.id = (v_file_id + 1) FOR UPDATE;
      pad_data := SUBSTR(pad_data, 0, 2 * v_pad_amount);
      DBMS_LOB.WRITEAPPEND(blob_locator, v_pad_amount, pad_data); -- append bytes at the end of blob
      COMMIT;
  ...
  CURSOR active_cmsdk_user_sessions IS
    SELECT USERID FROM IFSSYS.ODMZ_SESSION WHERE USERID = v_userid;
  CURSOR CMSDK_UserSpecific_Files IS
    SELECT name, id FROM ifssys.odm_publicobject WHERE owner = v_userid AND (name LIKE '%.enc');
  CMSDK_UserSpecific_Files_record CMSDK_UserSpecific_Files%ROWTYPE;
  BEGIN
    OPEN active_cmsdk_user_sessions;
    FETCH active_cmsdk_user_sessions INTO v_sessions; -- Only if no existing sessions of this user then        -- decrypt or encrypt
    IF active_cmsdk_user_sessions%NOTFOUND THEN
      ...
      LOOP                 -- decrypt, change keys and encrypt single user-owned files back with new keys
        FETCH CMSDK_UserSpecific_Files INTO CMSDK_UserSpecific_Files_record;
        EXIT WHEN CMSDK_UserSpecific_Files%NOTFOUND;
          decrypt (CMSDK_UserSpecific_Files_record.id + 1, v_dec_key);    -- decrypt with old decryption keys
          DELETE FROM CMSDK_UserSpecificFile_keys WHERE CMSDK_user_id = v_userid;
          INSERT INTO CMSDK_UserSpecificFile_keys (ifs_user_id, enc_key, dec_key) VALUES (v_userid, new_enc_key, new_dec_key);
          encrypt_User_Specific_file (CMSDK_UserSpecific_Files_record.id + 1, new_enc_key, v_userid, false);
      END LOOP;
      CLOSE CMSDK_UserSpecific_Files;
    ELSE
      dbms_output.put_line('User ' || userid ||' has active session');
    END IF;
    CLOSE active_ifs_user_sessions;
  ...
  v_fileid        INTEGER;
  blob_original   BLOB;                     -- the original blob in CMSDK tables
  v_media         INTEGER := 0;
  BEGIN
        ...
        SELECT media INTO v_media FROM ifssys.odm_contentobject WHERE ifssys.odm_contentobject.id = v_fileid + 1;
        IF (v_media = 2061) THEN
          SELECT nonindexedblob2 INTO blob_original FROM ifssys.odmm_nonindexedstore WHERE ifssys.odmm_nonindexedstore.id = (v_fileid + 2);
        ELSE
          SELECT globalindexedblob INTO blob_original FROM ifssys.odmm_contentstore WHERE ifssys.odmm_contentstore.id = (v_fileid + 2);
        END IF;
	  -- encrypt file, passing fileid and encryption key
        ...
        UPDATE CMSDK_MultiUserFile_keys_state  SET file_encrypted = 'TRUE'
          WHERE CMSDK_filepath LIKE v_filepath;
  ...
  -- is used to retrieve the temporary LOB segment from the CMSDK tables
  ...
  BEGIN
        ...
        v_fileid := split (v_filepath);
        -- find the location of the original blob
        SELECT templob INTO blob_original FROM CMSDK_MultiUserFile_keys_state WHERE CMSDK_filepath LIKE v_filepath;
        encrypt (v_fileid + 1, v_file_enc_key, blob_original);  -- encrypt file, passing fileid and encryption key
        -- update encrypted status of file
       UPDATE CMSDK_MultiUserFile_keys_state SET file_encrypted = 'TRUE', templob =NULL WHERE CMSDK_filepath LIKE v_filepath;
  END encrypt_Multi_user_file_sub;

-- This procedure is called upon user logon to the FSDB to decrypt multi-user files. The filepath is             -- passed as parameter which is used to retrieve the temporary LOB segment from the CMSDK tables
  ...
  BEGIN
        ...
        -- update encrypted status of file
        UPDATE CMSDK_MultiUserFile_keys_state  SET file_encrypted = 'FALSE', templob = blob_original
          WHERE CMSDK_filepath LIKE v_filepath;
...
-- This procedure is for spatial/ DBA administrator for final decryption of multi-user spatial file
  ...
  BEGIN
        ...
        decrypt (v_fileid + 1, v_file_dec_key);  -- decrypt file, passing fileid and decryption key
        -- update encrypted status of file
        UPDATE CMSDK_MultiUserFile_keys_state  SET file_encrypted = 'FALSE' WHERE CMSDK_filepath LIKE v_filepath;
        dbms_output.put_line('File ' || v_filepath ||' decrypted sucessfully');
  END decrypt_Multi_user_file_final;

-- This procedure is intended for spatial/ DBA administrator for changing encryption and decryption keys of -- multi-user spatial files
...
-- Check if multi-user file is in-use. If any user, who has priviledge on the multi-user file, has active   -- sessions we assume that the file would be decrypted already. In that case, a message is displayed to the    -- administrator.
  CURSOR in_session_files IS
    SELECT cmsdk_filepath FROM CMSDK_MultiUserFile_user_catalog
      WHERE cmsdk_user_id IN (SELECT userid FROM ifssys.odmz_session);
  in_session_files_record in_session_files%ROWTYPE;
  ...
  BEGIN
    ...
    IF (in_use = FALSE) THEN -- if file not in-use then encrypt back for protection
        ...
        DELETE FROM CMSDK_MultiUserFile_keys_state WHERE CMSDK_filepath = v_filepath;
        INSERT INTO CMSDK_MultiUserFile_keys_state (CMSDK_filepath,enc_key,dec_key) VALUES (v_filepath,new_enc_key,new_dec_key);
